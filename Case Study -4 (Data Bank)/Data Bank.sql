...
select count(distinct region_id) as unique_nodes from regions;  
...
-- Q2) What is the number of nodes per region?
  ...
  -- or
  select r.region_id,count(cn.node_id) as nodes_per_region from customer_nodes cn join regions r
  on cn.region_id=r.region_id group by r.region_id order by r.region_id;
  
-- Q3) How many customers are allocated to each region?
  select * from customer_nodes;  
  select r.region_id,count(cn.customer_id) as no_of_customers from customer_nodes cn join regions r
  on r.region_id=cn.region_id group by r.region_id order by r.region_id;

-- Q4) How many days on average are customers reallocated to a different node?  
  select * from customer_nodes;
  select customer_id,node_id,sum(datediff(end_date,start_date)) as days from customer_nodes
  group by customer_id,node_id;
  
  select count(distinct node_id) from customer_nodes;
  
  select customer_id,node_id,datediff(end_date,start_date) as days from customer_nodes;
  select max(end_date) from customer_nodes;
  
 select round(avg(datediff(end_date,start_date)),2) as avg_days from customer_nodes 
 where end_date<>(select max(end_date) from customer_nodes); 
  
-- B. CUSTOMER TRANSACTIONS.
-- 1) What is the unique count and total amount for each transaction type?
select * from customer_transactions;
select txn_type,count(*) as no_of_txns,sum(txn_amount) as total_txn_amount
from customer_transactions group by txn_type;

-- 2) What is the average total historical deposit counts and amounts for all customers?
select * from customer_transactions;
select customer_id,count(txn_type) as total_deposit_counts,sum(txn_amount) as total_deposit_amount
...
(
				(with x as 
				(
					select monthname(txn_date) as month,customer_id,
					count(txn_type) as no_of_deposits from customer_transactions
					where txn_type in ('deposit')
					group by month,customer_id
				) 
				select x.month,count(distinct x.customer_id) as no_of_customers from x
				where x.no_of_deposits>1 group by x.month order by x.month
                )
							   union all 
			    (
					with y as
					(
						select monthname(txn_date) as month,customer_id,txn_type,count(txn_type) as no_of_p_w
						from customer_transactions where txn_type='purchase'or txn_type='withdrawal'
						group by month,customer_id,txn_type order by month
					)
					select y.month,count(distinct y.customer_id) as no_customers
					from y where y.no_of_p_w<=1 group by y.month order by y.month
			    )

)
select month,sum(no_of_customers) as desired_customers from cte group by month;

with fact as
(
select customer_id,month(txn_date) as month_no,monthname(txn_date) as month_name,
sum(if(txn_type="deposit",1,0)) as deposit_count,
sum(if(txn_type="purchase",1,0)) as purchase_count,
sum(if(txn_type="withdrawal",1,0)) as withdrawal_count
from customer_transactions group by customer_id,month_no,month_name
)
select fact.month_no,fact.month_name,
count(distinct fact.customer_id) as no_of_customers from fact where 
fact.deposit_count>1 and (fact.purchase_count=1 or fact.withdrawal_count=1)
group by fact.month_no,fact.month_name;
...
(
	...
	from customer_transactions group by customer_id,month_name
)
select x.customer_id,x.month_name,x.total_deposit,x.total_purchase,x.total_withdrawal,
x.total_deposit-(x.total_purchase+x.total_withdrawal) as closing_balance
from x group by x.customer_id,x.month_name order by x.customer_id asc;
